...
| Processing | Durable processing jobs; immutable, retention-bound transcript artifacts; and an opt-in, provider-neutral, shell-free command adapter. The dormant adapter 1.1 path can pass an exact lease-bound transcript from `transcribe` to `align`, records successful consumption atomically, and then pauses before the undesigned `correlate` stage. [ADR-016](decisions/ADR-016-local-processor-isolation.md) defines the host runner/Compose profile for an operator-selected private-pilot processor. Its frozen isolation-1.0 wrappers can safely carry an explicitly selected nested local input/result 1.1 without changing topology or defaults. Normal backend startup is inert and default-deny for egress. | Production completion still creates only legacy pipeline 1.0 jobs. No transcription model, LLM, repository connector, telemetry connector, API provider, image, model, or command is selected. Pipeline 1.1 and its isolated path remain dormant unless an operator supplies the separate outer local-command 1.1 selection. The accepted profile is offline, read-only and resource-bounded, but the exclusive worker still runs only while the HTTP service is stopped and is not an unattended production worker. |
