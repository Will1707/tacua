...
| 2. Reviewer uses administrator API | Native or same-origin browser reviewer through the deployment TLS boundary to backend admin routes. | Native stores origin and secret atomically in device-only, when-unlocked secure storage. The browser requires the deployment's exact origin and uses tab-scoped `sessionStorage`; it cannot provide Keychain-equivalent protection. Setup and provider activation validate the operator-declared reviewer identity locally, authenticate its status-only binding, and only then read build launch metadata before persisting or exposing a client. New backends return no configured identity. During the bounded stagger from bootstrap 1.0, its exact legacy identity field must match the already-verified declaration and is discarded rather than used as configuration; transition authorization independently remains fail-closed. Both clients omit cookies, reject redirects, verify response origin, and bound responses ([reviewer README](../apps/reviewer/README.md), [browser boundary](../apps/reviewer/SELF_HOSTED_WEB.md)). The backend authenticates before reading protected request bodies and exposes bounded routes ([HTTP adapter](../services/backend/src/tacua_backend/http_api.py)). TLS, firewalling, rate limiting, and public exposure are deployment controls. |
| 3. Reviewer launches the tested app | Backend to reviewer app to a custom URL scheme in the QA app. Only an opaque, short-lived, one-use launch code crosses the deep-link boundary. | The code carries no evidence or reusable upload credential. An AppDelegate subscriber mirrors only links accepted by the build-pinned parser into a bounded, volatile, atomically drained inbox without claiming the callback or logging content. Consent must still produce a one-shot native handle before exchange ([launch parser](../experiments/ios-capture-spike/package/ios/TacuaLaunchLink.swift), [volatile inbox](../experiments/ios-capture-spike/package/ios/TacuaLaunchURLInbox.swift), [subscriber](../experiments/ios-capture-spike/package/ios/TacuaLaunchURLAppDelegateSubscriber.swift), [start lifecycle](../experiments/ios-capture-spike/package/ios/TacuaSDKStartLifecycle.swift)). |
...
| Spoofing | A caller impersonates the configured reviewer or operator. | C2 and C8 require the administrator bearer for admin routes, and reviewer setup verifies that its declared transition actor matches the single deployment pin. The shared secret still authenticates possession, not a named human, and supplies no MFA, per-action signing, or non-repudiation. |
...
| C2 | One-use launch grants; keyed server verifiers; scoped, expiring capability states; exact replay and rotation | [Credential store](../experiments/ios-capture-spike/package/ios/TacuaCredentialStore.swift), [start](../experiments/ios-capture-spike/package/ios/TacuaSDKStartLifecycle.swift) and [resume](../experiments/ios-capture-spike/package/ios/TacuaSDKResumeLifecycle.swift) journals, [protocol validator](../contracts/sdk-backend-protocol/src/protocol_contract.py), and backend [service](../services/backend/src/tacua_backend/service.py). | [Protocol tests](../contracts/sdk-backend-protocol/tests/test_protocol_contract.py), [SDK client tests](../experiments/ios-capture-spike/package/tests/SDKBackendClientTests.swift), backend [protocol/HTTP tests](../services/backend/tests/test_backend.py), and the real [SDK/backend loopback E2E](../experiments/ios-capture-spike/package/tests/sdk_backend_loopback_e2e.py). |
