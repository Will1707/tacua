...
| Reviewer app | A scoped reviewer client. The same-origin browser uses either an exact Tailscale Serve app capability or a revocable `HttpOnly` pairing cookie; native stores one revocable reviewer bearer, or temporarily the exact pairing cleanup token and client kind, in device-only secure storage. It does not receive the administrator secret, record another app, or read the tested app's sandbox. V1 still has no per-human MFA or hostile multi-tenant boundary. |
...
deployment. The administrator credential remains operator-only; reviewer
capabilities and sessions authorize a fixed reviewer role rather than a
...
| 2. Reviewer uses scoped API | Native or same-origin browser reviewer through the deployment TLS boundary to the fixed reviewer-route allowlist. | Serve may inject one exact configured app-capability value; otherwise one-use pairing creates a revocable 30-day session. Web receives only a `Secure`, `HttpOnly`, `SameSite=Strict` cookie and persists no configuration; native stores an exact V5 union of endpoint plus no credential, one scoped bearer, or one temporary pairing-cleanup credential in device-only, when-unlocked secure storage. The native cleanup token is committed before exchange and canceled/cleared before any restart probe; a verified bearer atomically replaces it. Unsafe operations require exact Origin and CSRF, while client startup requires the authenticated principal to match the exact bootstrap 1.0 compatibility identity before exposing an operational client. The compatibility field is discarded rather than used as configuration. Requests reject redirects, verify response origin, and bound responses ([reviewer README](../apps/reviewer/README.md), [browser boundary](../apps/reviewer/SELF_HOSTED_WEB.md)). The backend authenticates before reading protected bodies; administrator approval and arbitrary session revocation remain operator-only ([HTTP adapter](../services/backend/src/tacua_backend/http_api.py)). TLS, tailnet policy, firewalling, rate limiting, and public exposure are deployment controls. |
| 3. Reviewer launches the tested app | Backend to reviewer app to a custom URL scheme in the QA app. Only a complete backend-issued URL containing an opaque, short-lived, one-use launch code crosses the deep-link boundary. | Transport policy 1.2 seals the scheme into the SDK profile and build digest. The scoped launch-link route binds the build/session/scope and returns the exact URL; the reviewer validates and opens or renders those bytes without composing a scheme. The code carries no evidence or reusable upload credential. An AppDelegate subscriber mirrors only links accepted by the build-pinned parser into a bounded, volatile, atomically drained inbox without claiming the callback or logging content. Consent must still produce a one-shot native handle before exchange ([launch parser](../experiments/ios-capture-spike/package/ios/TacuaLaunchLink.swift), [volatile inbox](../experiments/ios-capture-spike/package/ios/TacuaLaunchURLInbox.swift), [subscriber](../experiments/ios-capture-spike/package/ios/TacuaLaunchURLAppDelegateSubscriber.swift), [start lifecycle](../experiments/ios-capture-spike/package/ios/TacuaSDKStartLifecycle.swift)). |
...
| Spoofing | A caller impersonates the configured reviewer or operator. | C2 and C8 keep the administrator bearer on operator-only routes. Reviewer routes require the exact Serve-injected app capability or a revocable scoped session, bind the authenticated principal to the single deployment reviewer pin, and require Origin plus CSRF for unsafe browser operations. These credentials still authenticate possession or tailnet policy membership, not a cryptographically named human, and supply no MFA, per-action signing, or non-repudiation. A trusted local process can bypass Serve and forge the capability header in the documented single-owner host profile. |
...
| C2 | One-use launch grants; keyed server verifiers; scoped, expiring capability states; exact replay and rotation | [Credential store](../experiments/ios-capture-spike/package/ios/TacuaCredentialStore.swift), [start](../experiments/ios-capture-spike/package/ios/TacuaSDKStartLifecycle.swift) and [resume](../experiments/ios-capture-spike/package/ios/TacuaSDKResumeLifecycle.swift) journals, [protocol validator](../contracts/sdk-backend-protocol/src/protocol_contract.py), backend [reviewer-auth store](../services/backend/src/tacua_backend/reviewer_auth_store.py), and backend [service](../services/backend/src/tacua_backend/service.py). | [Protocol tests](../contracts/sdk-backend-protocol/tests/test_protocol_contract.py), [SDK client tests](../experiments/ios-capture-spike/package/tests/SDKBackendClientTests.swift), backend [reviewer-auth tests](../services/backend/tests/test_reviewer_auth_store.py), backend [protocol/HTTP tests](../services/backend/tests/test_backend.py), and the real [SDK/backend loopback E2E](../experiments/ios-capture-spike/package/tests/sdk_backend_loopback_e2e.py). |
| C3 | Strict, bounded network and serialization boundary | SDK [redirect-rejecting client](../experiments/ios-capture-spike/package/ios/TacuaSDKBackendClient.swift), reviewer [client](../apps/reviewer/src/api/client.ts), same-origin [static reviewer](../services/reviewer-web/server.py), backend [HTTP adapter](../services/backend/src/tacua_backend/http_api.py), and closed [runtime](../contracts/runtime/src/runtime_contract.py) and [ticket](../contracts/ticket-candidate/src/ticket_candidate_contract.py) validators. | SDK [protocol tests](../experiments/ios-capture-spike/package/tests/SDKBackendProtocolTests.swift), the real [URLSession-to-backend loopback E2E](../experiments/ios-capture-spike/package/tests/sdk_backend_loopback_e2e.py), reviewer response-limit and web-config tests, static-server and image-input tests, backend [HTTP tests](../services/backend/tests/test_backend.py), and contract suites under [`contracts`](../contracts). |
...
| C7 | Default-deny, opt-in processing with bounded data inheritance and isolated selected-model execution | Checked-in [internal Compose network](../services/backend/compose.yaml), [processing adapter](../services/backend/src/tacua_backend/processing_adapter.py), [worker](../services/backend/src/tacua_backend/processing_worker.py), [ADR-016 isolated profile](decisions/ADR-016-local-processor-isolation.md), [Compose profile](../services/backend/compose.processor.yaml), [host runner](../services/backend/scripts/run_isolated_processor.py), [ADR-020 state bridge](decisions/ADR-020-compose-state-processing-bridge.md), [Compose bridge gate](../services/backend/scripts/run_compose_isolated_processing.py), and [bridge runbook](../services/backend/COMPOSE_PROCESSING_BRIDGE.md). The one-shot state worker transfers only already-open descriptors over one private Unix socket; neither it nor the selected processor receives the Docker socket. No command, model, connector, provider key, or automatic runner is configured. | [Adapter tests](../services/backend/tests/test_processing_adapter.py), isolated profile/runner [tests](../services/backend/tests/test_isolated_processor.py), Compose-bridge [tests](../services/backend/tests/test_compose_processing_bridge.py), [job tests](../services/backend/tests/test_processing_jobs.py), and [publication tests](../services/backend/tests/test_processing_publication.py). |
...
| C9 | Hardened single-node packaging and fail-closed operator preflight | Non-root digest-pinned backend and [reviewer](../services/reviewer-web/Dockerfile) Dockerfiles, read-only/capability-dropped/default-deny [Compose model](../services/backend/compose.yaml), immutable-image [production override](../services/backend/compose.production.yaml), and [operator preflight](../services/backend/src/tacua_backend/operator_tool.py). | [Operator tests](../services/backend/tests/test_operator_tool.py), backend and reviewer image-input validators with adversarial tests, static reviewer container smoke tests, and the repository [verification workflow](../.github/workflows/verify.yml). |
