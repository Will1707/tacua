...
| Upload followed by asynchronous processing | The native SDK queue/client and lifecycle tests are in the iOS package; durable jobs, transcript artifacts, and consumption receipts are in [`processing_jobs.py`](../services/backend/src/tacua_backend/processing_jobs.py); canonical synthetic adapter and isolated-wrapper wires are in [`contracts/local-processing`](../contracts/local-processing/README.md); and the opt-in boundary is documented in the [processing-adapter runbook](../services/backend/PROCESSING_ADAPTER.md), [ADR-016](decisions/ADR-016-local-processor-isolation.md), and [ADR-019](decisions/ADR-019-processing-artifact-consumption.md). | Retry-safe upload/completion and durable asynchronous job state are implemented. Upload is currently foreground and process-bound, so relaunch must drain the durable queue again. Dormant adapter 1.1 can pass a verified transcript through alignment and then pauses; the fixture corpus freezes that wire but activates nothing. Production still creates only legacy pipeline 1.0 jobs, and the isolated runner is not yet 1.1-capable. An operator-selected private-pilot processor has a separate-UID, network-none, read-only, resource-bounded container gate, while no real image, model, transcription, research, or ticket-generation command is selected or bundled. The worker remains an exclusive offline operator action, not an unattended service. |
