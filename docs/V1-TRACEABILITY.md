...
| App-only ReplayKit narrated manual QA, at most 30 minutes | [`TacuaCaptureSession.swift`](../experiments/ios-capture-spike/package/ios/TacuaCaptureSession.swift), [`CapturePolicy.swift`](../experiments/ios-capture-spike/package/ios/CapturePolicy.swift), their Swift tests, the [ADR-018 machine gate](decisions/ADR-018-v1-app-audio-acceptance.md), and the bounded [physical-device results](../experiments/ios-capture-spike/PHYSICAL-DEVICE-RESULTS.md). | ReplayKit video, app audio, required microphone narration, segmentation, gaps, and a 30-minute active-capture design limit are implemented. V1 accepts at most 0.2% app-audio append drops only when every drop is an explicit gap. The historical device rate was about 0.156%, but its 121 drops were not gap-accounted, so new physical evidence remains required. The persisted monotonic deadline is not a claim of uninterrupted wall-clock enforcement while iOS suspends the process, nor is it a supported-device matrix. |
...
- produce a new 30-minute physical ADR-018 artifact at or below 0.2% in which
  every app-audio drop is recorded exactly once as a gap and the artifact
  validates against the exact digest- and identity-bound schema-4 manifest,
  then repeat the full
  capture-to-approved-ticket flow on supported physical devices;
